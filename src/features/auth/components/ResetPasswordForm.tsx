 ...
 * 
 * Native password reset form for the employment platform.
...
export function ResetPasswordForm({ onBackToLogin }: ResetPasswordFormProps) {
  const { requestPasswordReset, status, error, clearError } = useAuthContext()
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        {successMessage && (
          <div className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-sm rounded-md p-3">
            {successMessage}
          </div>
        )}
