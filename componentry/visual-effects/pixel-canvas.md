...
```bash
npx componentry@latest add pixel-canvas
```
...
|------|------|---------|-------------|
| `gap` | `number` | `6` | Size of the pixel cells in pixels. |
| `speed` | `number` | `0.02` | Decay speed of the pixel trail. Lower values create longer trails. |
| `colors` | `string[]` | `pink to cyan gradient` | Array of hex colors for the gradient. Pixels interpolate through these colors as they fade. |
| `variant` | `"default" | "trail" | "glow"` | `"default"` | Visual variant: "default", "trail" (rounded pixels), or "glow" (adds glow effect). |
| `noFocus` | `boolean` | `false` | Disables mouse/touch tracking. |
...
_Component from [Componentry](https://componentry.fun/docs/components/pixel-canvas)_
