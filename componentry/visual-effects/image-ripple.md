...
```bash
npx componentry@latest add image-ripple-effect
```
...
|------|------|---------|-------------|
| `images` | `RippleImageItem[]` | `gradient demo images` | Image descriptors. Each item accepts src, x, y, widthScale and heightScale. |
| `brushTextureUrl` | `string` | `generated radial brush` | Brush texture used for displacement stamps. |
| `distortionStrength` | `number` | `0.075` | How strongly waves distort sampled image UVs. |
| `waveCount` | `number` | `100` | Pool size for simultaneous ripple waves. |
| `waveSize` | `number` | `48` | Base wave stamp plane size. |
| `waveRotationSpeed` | `number` | `0.025` | Per-frame rotation speed of each visible wave. |
| `waveFadeMultiplier` | `number` | `0.95` | Opacity multiplier applied per frame (lower fades faster). |
| `waveGrowth` | `number` | `0.155` | How fast each wave expands while fading. |
| `waveSpawnThreshold` | `number` | `0.1` | Minimum pointer delta required to spawn a new ripple. |
| `className` | `string` | - | Additional classes for the root container. |
| `children` | `ReactNode` | - | Optional overlay content rendered above the canvas. |

---

_Component from [Componentry](https://componentry.fun/docs/components/image-ripple-effect)_
