...
```bash
npx componentry@latest add infinite-image-field
```
...
|------|------|---------|-------------|
| `images` | `string[]` | `12 built-in sci-fi Unsplash images` | Array of image URLs tiled across the infinite canvas. Images repeat in a deterministic pattern so the same grid cell always shows the same photo. |
| `imageWidth` | `number` | `300` | Width of each image tile in CSS pixels. |
| `imageHeight` | `number` | `200` | Height of each image tile in CSS pixels. |
| `gap` | `number` | `28` | Spacing between adjacent tiles in CSS pixels. |
| `maxSpeed` | `number` | `5` | Maximum pan speed when the cursor is at the edge of the canvas (pixels per frame). |
| `smoothing` | `number` | `0.07` | Lerp factor controlling how quickly velocity catches up to the cursor direction. Lower values feel heavier and dreamy; higher values feel snappy. |
| `borderRadius` | `number` | `0` | Corner radius applied to each image tile. |
| `className` | `string` | - | Additional classes applied to the root container. |

---

_Component from [Componentry](https://componentry.fun/docs/components/infinite-image-field)_
