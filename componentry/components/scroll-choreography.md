...
```bash
npx componentry@latest add scroll-choreography
```
...
|------|------|---------|-------------|
| `images` | `{ topLeft: string; topRight: string; bottomLeft: string; bottomRight: string; }` | - | An object containing the image URLs for each of the four quadrants. Required. |
| `className` | `string` | - | Additional CSS classes for styling the container. |

---

_Component from [Componentry](https://componentry.fun/docs/components/scroll-choreography)_
