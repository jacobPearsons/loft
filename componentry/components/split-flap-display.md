...
```bash
npx componentry@latest add split-flap-display
```
...
|------|------|---------|-------------|
| `text` | `string` | - | Simple text to display in a single row. |
| `rows` | `SplitFlapRow[]` | - | Array of { label, value } objects for a multi-row dashboard board. |
| `columns` | `number` | `14` | Total number of character cells per row. |
| `size` | `"sm" | "md" | "lg"` | `"md"` | Size variant for the cells. |
| `accentColor` | `string` | `"#22c55e"` | Color of the side indicator strips. |
| `showIndicators` | `boolean` | `true` | Whether to show the colored indicator strips on each row. |
| `staggerDelay` | `number` | `30` | Delay in ms between each character starting its flip animation. |
| `flipSpeed` | `number` | `35` | Speed in ms per character flip step. |
| `className` | `string` | - | Additional CSS classes for the outer container. |

---

_Component from [Componentry](https://componentry.fun/docs/components/split-flap-display)_
